...
def average(id_user,id_asset, status_operation, type_operation):
    if((type_operation == False) or (type_operation == 'False')):
        #buy average
        for p in Transactions.objects.raw('SELECT app_transactions.opening_price FROM app_transactions WHERE app_transactions.type_operation = False AND app_transactions.user_id_id = id_user AND app_transactions.asset_id= id_asset;'):
            print(p)
